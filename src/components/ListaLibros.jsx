...
const ListaLibros = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
        ...
        {libros.map(libro => (
          <div key={libro.idLibro} className="flex flex-col bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 group">
            ...
            <div className="p-4 flex flex-col flex-grow">
              <h3 className="font-bold text-gray-900 text-sm line-clamp-2 mb-1 min-h-[40px]">{libro.titulo}</h3>
      ...
      {libros.length === 0 && (
        <div className="text-center py-20 text-gray-400">No hay libros disponibles en este momento.</div>
      )}
    </div>
  );
};
