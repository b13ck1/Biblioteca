...
const FormularioLibro = ({ libroEdit, onSave, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Enviamos el objeto libro con la URL de la imagen actualizada
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-xl max-w-2xl mx-auto border border-gray-100">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* ... (campos de título, autor, género, precio, stock se mantienen igual) */}
  ...
  >
    ...
    {generos.map(g => (
      <option key={g.idGenero} value={g.idGenero}>{g.nombre}</option>
        ...
        {/* CAMBIO: Selector de Archivo en lugar de URL de texto */}
        <div className="md:col-span-2">
        ...
        <div className="md:col-span-2 mt-4 flex gap-3">
          ...
          {libroEdit && (
            <button type="button" onClick={onCancel} className="flex-1 bg-gray-200 text-gray-700 py-3 rounded-xl font-bold hover:bg-gray-300 transition">
              Cancelar
            </button>
          )}
        </div>
